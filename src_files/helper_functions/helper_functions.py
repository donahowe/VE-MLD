...
class LEAFDetection(datasets.coco.CocoDetection):
    def __init__(self, root, annFile, transform=None, target_transform=None, channel=1):
        with open(annFile) as file:
            description = json.load(file)
        self.ids = [element['id'] for element in description]
    ...
    def __getitem__(self, index):
        ...
        description = self.file
        #convert target
        target = description[index]["labels"]
        output = torch.zeros((self.channel, 6), dtype=torch.long)
        for obj in target:
            if self.channel >=3:
                output[2][obj] = 1
            else:
